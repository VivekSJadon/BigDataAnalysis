...
# DataFrame to store high radiation points
high_radiation_df = pd.DataFrame(columns=['captured_time', 'latitude', 'longitude', 'continent', 'value'])
...
def fetch_data(offset=0, limit=50, continent='all'):
    ...
    global high_radiation_df
    for row in result:
        row["value"] = float(row["value"])
        if row['value'] > 45:
            high_radiation_df = high_radiation_df.append(row, ignore_index=True)

    high_radiation_df.to_csv('high_radiation_points.csv', index=False)
